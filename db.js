...
/**
 * Fetch registrations: Supabase is the SINGLE SOURCE OF TRUTH.
 * When Supabase is connected and returns data, localStorage is overwritten
 * to mirror it exactly, so local and online always stay in sync.
 * localStorage is only used as a fallback when Supabase is unreachable.
 */
  ...
  if (supabaseClient) {
    try {
      ...
      if (!error && Array.isArray(data)) {
        // Supabase is the truth — sync localStorage to match it exactly
      ...
      console.warn('Supabase fetchDbRegistrations exception (falling back to local):', e);
    }
  }

  // Fallback: Supabase unreachable, use whatever is in localStorage
...
async function addDbRegistration(regData) {
  ...
  if (supabaseClient) {
    try {
      ...
      if (!error && data && data[0]) {
        newReg.id = data[0].id;
        newReg.created_at = data[0].created_at || newReg.created_at;
      }
    } catch (e) {
  ...
  // Also add to localStorage cache
